...
import { toolsCategory } from "../../../../category";
...
const buildUrlEntry = async (locale: Language, path: string, index: number): Promise<string> => {
    const links = await Promise.all(LANGUAGE_CODES.map(async (alternateLocale) => {
        const alternateCategory = await getContent(toolsCategory, alternateLocale);
...
export const GET: APIRoute = async ({ props }) => {
    ...
    const category = await getContent(toolsCategory, locale);
